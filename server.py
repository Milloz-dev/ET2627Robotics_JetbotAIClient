...
# ─────────────────────────────────────────────────────────────────────────────
# HLS colour ranges for detection
#   OpenCV HLS channel order: [H, L, S]
#   H: 0-180  L: 0-255 (lightness)  S: 0-255 (saturation)
#
...
# Green: H=40-90, L=20-220 (avoid very dark/very bright), S=80+
# OpenCV HLS scale: H=0–180, L=0–255, S=0–255
GREEN_HLS_LOWER = np.array([ 50,  90,  30])
GREEN_HLS_UPPER = np.array([ 85, 140, 70])

# ─────────────────────────────────────────────────────────────────────────────
# Red target (wraps around H=0): H=0-10 and H=160-180, L=30-220, S=100+
# Red target — two ranges because hue wraps around 0
# Sampled: H=2-3 and H=179-180 (OpenCV), L≈110, S≈75

TARGET_HLS_LOWER1 = np.array([  0,  60,  60])
TARGET_HLS_UPPER1 = np.array([ 10, 180, 255])

TARGET_HLS_LOWER2 = np.array([168,  60,  60])
TARGET_HLS_UPPER2 = np.array([180, 180, 255])

# ── Yellow: H=18-35, L=30-220, S=100+ ──────────────────────────────────────
YELLOW_HLS_LOWER = np.array([20,  100, 60])
YELLOW_HLS_UPPER = np.array([35, 150, 255])
 
# ── Blue: H=100-130, L=20-200, S=40+ ────────────────────────────────────────
# Blue: H≈108 in OpenCV (217°/2), L≈89, S≈110
BLUE_HLS_LOWER = np.array([ 100,  20,  60])
BLUE_HLS_UPPER = np.array([130, 40, 255]) 

BLUE_HLS_LOWER = np.array([100,  70,  70])
BLUE_HLS_UPPER = np.array([130, 150, 140])

# ── Purple: H=130-158, L=20-200, S=80+ ──────────────────────────────────────
PURPLE_HLS_LOWER = np.array([130, 20, 20])
PURPLE_HLS_UPPER = np.array([195, 200, 150]) 
PURPLE_HLS_LOWER = np.array([120,  60,  20])
PURPLE_HLS_UPPER = np.array([140, 150, 100])

# PURPLE_HLS_LOWER = np.array([125,  80,  30])
# PURPLE_HLS_UPPER = np.array([135, 130,  80])


# ── Reduce minimum area significantly ──────────────────────────────────────
MIN_CONTOUR_AREA  = 2500   # distant objects are tiny
...
def _detect_color(hls, lower, upper, frame_h, frame_w,
                  lower2=None, upper2=None, real_height_cm=OTHER_REAL_HEIGHT_CM):
    # Blur the HLS image to merge small distant fragments
    hls_blurred = cv2.GaussianBlur(hls, (7, 7), 0)
    mask = cv2.inRange(hls_blurred, lower, upper)
    if lower2 is not None:
        mask = cv2.bitwise_or(mask, cv2.inRange(hls_blurred, lower2, upper2))
...
def detect_objects(frame):
    h, w = frame.shape[:2]
    # Convert BGR -> HLS (OpenCV channel order: Hue, Lightness, Saturation)
    hls  = cv2.cvtColor(frame, cv2.COLOR_BGR2HLS)
 
    # ── Red target (27 cm tall) ──────────────────────────────────────────────
    red_blobs = _detect_color(hls,
                              TARGET_HLS_LOWER1, TARGET_HLS_UPPER1, h, w,
                              lower2=TARGET_HLS_LOWER2, upper2=TARGET_HLS_UPPER2,
                              real_height_cm=TARGET_REAL_HEIGHT_CM)
    target = red_blobs[0] if red_blobs else None
 
    # ── Green objects (14.5 cm) ──────────────────────────────────────────────
    greens  = _detect_color(hls, GREEN_HLS_LOWER,  GREEN_HLS_UPPER,  h, w,
                            real_height_cm=OTHER_REAL_HEIGHT_CM)
 
    # ── Yellow objects (14.5 cm) ─────────────────────────────────────────────
    yellows = _detect_color(hls, YELLOW_HLS_LOWER, YELLOW_HLS_UPPER, h, w,
                            real_height_cm=OTHER_REAL_HEIGHT_CM)
 
    # ── Blue objects (14.5 cm) ───────────────────────────────────────────────
    blues   = _detect_color(hls, BLUE_HLS_LOWER,   BLUE_HLS_UPPER,   h, w,
                            real_height_cm=OTHER_REAL_HEIGHT_CM)
 
    # ── Purple objects (14.5 cm) ─────────────────────────────────────────────
    purples = _detect_color(hls, PURPLE_HLS_LOWER, PURPLE_HLS_UPPER, h, w,
                            real_height_cm=OTHER_REAL_HEIGHT_CM)
...
def annotate_frame(frame, detection):
    ...
    _draw_boxes(out, detection["greens"],   (0, 200,   0), "GRN")
